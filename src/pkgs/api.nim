...
## Client setup code
template requestWrapper(client : HttpClient, url: string, reqtype: ReqType = Get, action: untyped) =
    ...
    while true:
        ...
        try:
            ...
            var 
                body {.inject.} : string
                status {.inject.} : string
                headers {.inject.} : TableRef[string, seq[string]]
                resp: Response
...
proc paramsToQuery(params: TimeLineParams): string =
    ...
    for name, value in params.fieldPairs():
        ...
        when name != "kind":
            ...
            result.add(fmt"$1=$2&".format(@[name, $value]))
...
proc setReqHeaders*(client: var HttpClient) =
    ...
    requestWrapper client, COOKIEURL, Get:
...
proc getUser*(client: HttpClient, username: string): Option[User] =
    ...
    info(fmt"Getting {username} user information")

    let variables : TimeLineParams = block :
        
        var variables : TimeLineParams = genParams(kind = UserParam)
        variables.screen_name = username.toLowerAscii

        variables

    requestWrapper client, USERINFOURL.format(encodeUrl($(variables.toJson))), Get:
...
proc getUserTweets*(client: HttpClient, user: User, count: int = 10, replies: bool = false): seq[Tweet] =
    ...
    let variables : TimeLineParams = block :
        ...
        var variables : TimeLineParams = genParams(kind = TweetParam)
    ...
    requestWrapper client, USERTWEETURL.format(encodeUrl($(variables.toJson))), Get:
        ...
        if status == "200 OK":
            ...
            try:
                ...
                for instruction in userinfoJson{"data", "user", "result", "timeline", "timeline",
                        "instructions"}.getElems:
...
proc searchUsers*(client: HttpClient, keyword: string, count: int = 10): seq[User] =
    ...
    let variables : TimeLineParams = block : 
        ...
        var variables : TimeLineParams = genParams(kind = SearchParam)
        variables.q = keyword
    ...
    requestWrapper client, SEARCHUSERS.format(paramsToQuery(variables)), Get:
...
proc searchTweets*(client: HttpClient, keyword: string, count: int = 10): seq[Tweet] =
    ...
    requestWrapper client, SEARCHTWEETS.format(paramsToQuery(variables)), Get:
